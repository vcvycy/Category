...
class TextRCNN:
    def __init__(self, sequence_length, num_classes, vocab_size, word_embedding_size, context_embedding_size,
        ...
        print("============text_length: %s" %(text_length))
        # Embeddings
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            self.W_text = tf.Variable(tf.random_uniform([vocab_size, word_embedding_size], -1.0, 1.0), name="W_text",trainable=W_text_trainable)
        ...
        with tf.name_scope("output"):
            ...
            #self.logits =tf.nn.softmax(tf.nn.xw_plus_b(self.y3, W4, b4, name="logits"))
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = tf.reduce_mean(losses) + tf.add_n([tf.nn.l2_loss(v) for v in vars])*l2_reg_lambda
        ...
        #trainop step summary
        self.global_step = tf.Variable(0, name="global_step", trainable=False)
        self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss, global_step=self.global_step)
        
        # Summaries for loss and accuracy
        if out_dir!=None:
            dev_loss_summary = tf.summary.scalar("dev_loss", self.loss)
            dev_acc_summary = tf.summary.scalar("dev_accuracy", self.accuracy)
            ...
            self.dev_summary_op = tf.summary.merge([dev_loss_summary, dev_acc_summary]) 
            # Dev summaries
